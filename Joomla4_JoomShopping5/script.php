 ...
 * @since       3.0
 */
class plgJoomshoppingGercpayInstallerScript
{
    /**
     * @var string
     * @since 3.0.0
    ...
    {
        $this->installPath = JPATH_ROOT . '/components/com_jshopping/payments/pm_gercpay';
    }
    ...
    {
        // JoomShopping GercPay plugin install path.
        ...
        foreach ($this->files as $filename) {
            if (is_array($filename)) {
                foreach ($filename as $language) {
                    copy(
                        JPATH_PLUGINS . "/joomshopping/gercpay/language/$language",
                        $this->installPath . "/language/$language"
                    );
                }
            } else {
                copy(
                    JPATH_PLUGINS . "/joomshopping/gercpay/$filename",
        ...
        $query->select($db->quoteName('payment_code'))
            ...
            ->where($db->quoteName('payment_code') . ' = '. $db->quote('Gercpay'));
        $db->setQuery($query);
        ...
        $sqlInstall = JPATH_PLUGINS . '/joomshopping/gercpay/sql/install.sql';
