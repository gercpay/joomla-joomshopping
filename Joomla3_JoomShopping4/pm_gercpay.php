...
class pm_gercpay extends PaymentRoot
{
    ...
    public function showEndForm($pmconfigs, $order)
    {
        ...
        $description = GERCPAY_ORDER_DESCRIPTION . ' ' . $_SERVER["HTTP_HOST"] . ', '
        ...
        $gercpay_args['signature'] = $this->getSignature($gercpay_args, $this->keysForSignature, $pmconfigs['gercpay_secret_key']); ?>
        <html>
        <head>
            <meta http-equiv="content-type" content="text/html; charset=utf-8"/>
        </head>
        <body>
        <form id="paymentform" action="<?php print pm_gercpay::URL; ?>" name="paymentform" method="post">
            <?php
            foreach ($gercpay_args as $key => $value) :
                ?>
        ...
        if ($rescode != 'notify') {
            ...
            if (isset($getParams['result'])) {
                switch (strtolower($getParams['result'])) {
                    case 'success':
                        $values->msg = JText::_(PLG_JOOMSHOPPING_GERCPAY_SUCCESS);
                        $values->type = 'success';
                        ...
                        $cart->load();
                        $cart->deleteAll();
                        $redirect_url = $pmconfig["gercpay_approve_url-$language"] ?? '';
                        break;
                    case 'fail':
                        $values->msg = JText::_(PLG_JOOMSHOPPING_GERCPAY_FAIL);
                        $values->type = 'error';
                        $redirect_url = $pmconfig["gercpay_decline_url-$language"] ?? '';
                        break;
                    case 'cancel':
                        $values->msg = JText::_(PLG_JOOMSHOPPING_GERCPAY_CANCEL);
                        $values->type = 'warning';
                        $redirect_url = $pmconfig["gercpay_cancel_url-$language"] ?? '';
                        break;
                    default:
                        $values->msg = JText::_(PLG_JOOMSHOPPING_GERCPAY_UNKNOWN_ERROR);
                        $values->type = 'error';
                        $redirect_url = $pmconfig["gercpay_decline_url-$language"] ?? '';
                }
    ...
    public function isPaymentValid($response, $pmconfig, $order)
    {
        ...
        if ($orderId !== $order->order_id || !isset($response['transactionId'])) {
            throw new Exception(GERCPAY_UNKNOWN_ERROR);
        }

        if ($pmconfig['gercpay_merchant_id'] !== $response['merchantAccount']) {
            throw new Exception(GERCPAY_MERCHANT_DATA_ERROR);
        }

        $verificationSignature = $this->getSignature($response, $this->keysForResponseSignature, $pmconfig['gercpay_secret_key']);
        if ($verificationSignature !== $response['merchantSignature']) {
            throw new Exception(GERCPAY_SIGNATURE_ERROR);
        }
        ...
        throw new Exception(GERCPAY_ORDER_DECLINED);
    }
